final: prev: {
  warcraftlogs = prev.stdenv.mkDerivation rec {
    ...
    installPhase = ''
      ...
      cp $src $out/bin/warcraftlogs.appimage
      chmod +x $out/bin/warcraftlogs.appimage

      # Create wrapper script for clean launch
      makeWrapper ${prev.appimage-run}/bin/appimage-run $out/bin/warcraftlogs \
        --add-flags "$out/bin/warcraftlogs.appimage" \
      ...
      cat > $out/share/applications/warcraftlogs.desktop << EOF
[Desktop Entry]
Type=Application
Name=Warcraft Logs
Comment=Upload and analyze World of Warcraft combat logs
Exec=$out/bin/warcraftlogs
Icon=warcraftlogs
Categories=Game;
StartupWMClass=warcraftlogs
EOF

      # Extract and install icon
      ${prev.appimage-run}/bin/appimage-run --appimage-extract-and-run $out/bin/warcraftlogs.appimage --help > /dev/null 2>&1 || true
      ...
      if [ -d squashfs-root ]; then
        ...
        if [ -f $out/share/icons/hicolor/256x256/apps/*.png ]; then
          mv $out/share/icons/hicolor/256x256/apps/*.png $out/share/icons/hicolor/256x256/apps/warcraftlogs.png
        fi
        rm -rf squashfs-root
      fi

      # Create a simple fallback icon if extraction failed
      if [ ! -f $out/share/icons/hicolor/256x256/apps/warcraftlogs.png ]; then
        # Create a simple colored rectangle as fallback
        echo "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChwGA60e6kgAAAABJRU5ErkJggg==" | base64 -d > $out/share/icons/hicolor/256x256/apps/warcraftlogs.png
      fi
    ...
    meta = with prev.lib; {
      description = "Upload and analyze World of Warcraft combat logs";
